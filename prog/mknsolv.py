...
fnma = "nsolv.ma" # output: Mathematica script
fnroot = "nsolv.txt" # root file, output of the Mathematica script
fnrootT = "solvT.txt" # special root file for the logistic map
...
def usage():
  ...
  print "  %s [OPTIONS] input" % sys.argv[0]
  print """
    Write a Mathematica script for numeric solutions of the logistic and cubic maps
    ...
    -o, --output    output Mathematica script, default: {{fnma}}
    -r, --root      root file, default: {{fnroot}}
    -R, --rootT     extra root file of the logistic map, default: {{fnrootT}}
    ...
    -T, --tmroot    the maximal time for FindRoot[]
    ...
    The output is {{fnma}}, to run it type

      math < {{fnma}}

    which writes {{fnroot}} and possibly {{fnrootT}} (logistic map).
  ...
  """.replace("{{fnma}}", fnma
    ).replace("{{fnroot}}", fnroot
    ).replace("{{fnrootT}}", fnrootT
    ).replace("{{fneq}}", fneq
    ).replace("{{fnbra}}", fnbra
    )
  exit(1)
...
def doargs():
  ...
  try:
    opts, args = getopt.gnu_getopt(sys.argv[1:], "ho:r:R:q:b:p:w:0:1:T:W:v",
         [ "help",
           "output=", "eq=", "root=", "rootT", "bra=",
  ...
  global fninp, fnma, fneq, fnroot, fnrootT, fnbra
...
fnroot = "{{fnroot}}";  (* output root file *)
fnrootT = "{{fnrootT}}";  (* output root file *)
fnbra = "{{fnbra}}";
...
solve[poly_, x_, prec_ : precdef, wprec_ : None, fnbra_ : {{fnbra}},
      k0_ : None, k1_ : None, fnroot_ : None, fnrootT_ : None, verbose_ : None,
      nwarmup_ : {{nwarmup}}] :=
    Module[{intv, k, ls = {}, sol, x1, x2, y, kmin = k0, kmax = k1, err,
            s, rv, fpT, dydx, tm},

  (* I. compute or load bracket file *)
  ...
  (* II. determine which roots to compute *)
  If [ kmin === None, kmin = 1 ];
  ...
  For [ k = kmin, k <= kmax, k++,
    ...
    (* 1. try to narrow down the bracket and/or do some basic checking,
       s.t. the derivative makes sense, this _may_ make FindRoot[] easier *)
    If [ nwarmup >= 0,
      tm = Timing[
    ...
    err = True;
    (* 2. try to use the built-in Mathematica function FindRoot[] first *)
    err = False;
    ...
    (* 4. write the standard output file *)
    sol = N[sol, prec];
    ls = Append[ls, sol];
    Print["sol ", k, " ", sol, " (", N[x1], ", ", N[x2], ") ", fnroot];
    If [ !(fnroot === None),
      xsave[fnroot, {k, sol, x1, x2}, True];
    ];

    (* 5. write the special output of the logistic map *)
    If [ !(fnrootT === None),
      rv = 1 + Sqrt[1 + sol];
      s = "";
      If [ k === kmin,
...
tm = Timing[
  ...
  kmin, kmax, fnroot, fnrootT, verbose];
][[1]];
Print["solve: ", tm];
"""
...
if __name__ == "__main__":
  ...
  if fninp.startswith("fit"):
    print "Warning: cannot determine if %s is logistic or cubic, will assume logistic" % fninp
