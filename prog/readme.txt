...
hen.ma            Henon map
mkinterp.py       interpolation code
...
per10.nb  using integer preserving Gaussian eliminations to solve the equations
   * featuring cycsolve8[] using gedet0[],
   on T60, for just the onset eq. used 10s vs. 20s of cycsolve7[] for n = 8, used 200s for n = 9, 4563s (76 min) for n = 10
           for both eqs. used 13s for n = 8, 240s for n = 9.
   unfortunately, we still have no way to solve the discriminant/resultant effectively, which takes much longer.
