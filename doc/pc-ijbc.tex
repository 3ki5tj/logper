...
\tbl{
Square-free cyclic polynomials $C_p(\vx)$ for the logistic map ($n \ge 5$).}
%\begin{center}
{
...
%\begin{remark}[1]
  ...
  to any $\lambda = \exp(i\phi)$,
  where $\phi \in [0, 2\pi]$,
  the algorithm still applies.
  By increasing $\phi$ from 0 to $2\pi$,
...
$^\dagger$
$\NB(n) = (1/n) \sum_{d|n} \phi(n/d) 2^d$ [\refeq{necklace}]
...
with $\lambda$ being $\exp(i\phi)$ \big[$\phi \in (0, 2\pi)$\big]
  instead of $\pm1$;
...
\begin{figure}[h]
  ...
  with a complex $R$;
  obtained from $P_n(R, e^{i\phi}) = 0$ with $\phi \in [0, 2\pi]$,
...
%\caption{
\tbl{
Two ways of counting the $N(4) = 6$ necklaces for $n = 4$.
}
%\begin{center}
{
...
%
We then weight them by the Euler's totient function $\phi(n/d)$.
%
Here, $\phi(m)$ gives the number of integers from 1 to $m$
...
We repeat the process over other divisors $d$ of $n$,
  and the resulting sum is $\sum_{d|n} \phi(n/d) 2^d$.
%
...
\begin{equation}
m \sum_{m|d, d|n} \phi(n/d) =
 m \sum_{\frac{n}{d}|\frac{n}{m}} \phi(n/d) = m \,  \frac{n}{m}  = n,
\label{eq:mphind}
\end{equation}
where we have used the identity $\sum_{d'|m'} \phi(d') = m'$,
with $d' = n/d$ and $m' = n/m$.
%
Summing over the necklaces yields $N(n) \, n$. Thus,
$N(n) = (1/n) \sum_{d|n} \phi(n/d) 2^d$.
...
\begin{equation}
  \NB(n) = N(n) = (1/n) \sum_{d|n} \phi(n/d) 2^{\,d},
\label{eq:necklace}
\end{equation}
%
...
\sum_{d | n} L(d)
  ...
  &= (1/n) \sum_{c \, | \, n} 2^{\,c} \, \phi(n/c)
  = N(n),
\end{align*}
where we have used $\phi(m) = \sum_{d' | m} \mu(\frac{m}{d'}) \, d'$,
which is the inversion of $m = \sum_{d' | m} \phi(d')$,
and \refeq{necklace}.
%
...
\begin{subequations}
\begin{align}
\deg_R A_n\RX &= \sum_{d|n} \phi(n/d) 2^{d-1}, \\
\deg_R P_n\RX &= \sum_{d|n} \mu(n/d) 2^{d-1} \equiv \beta(n), \\
\deg S_n(R) &= \beta(n) - \sum_{d | n, d < n} \beta(d) \, \phi(n/d).
\end{align}
\label{eq:degR}
\end{subequations}
\label{thm:degR}
...
\begin{table}[h]\footnotesize
%\caption{
\tbl{Onset and bifurcation polynomials of the $n$-cycles of the H\'enon map.}
...
%\[
%  N_3(n) = (1/n) \sum_{d|n} \phi(n/d) 3^d.
%\]
%
...
is given by
\begin{equation}
  N_e(n) = \frac{1}{n} \sum_{c d = n} \phi(c)
    \left[
...
So
\[
  N_e(n) \cdot n = \sum_{cd = n} T(d, c) \, \phi(c),
\]
...
The generating function $G_{\phi,\odd}(s)$
of $\phi(n) \odd(n)$ can be computed by taking the generating function
of both sides of the identity
\[
  n \, \odd(n)  = \sum_{d|n} \phi(d) \, \odd(d) \; \odd(n/d),
\]
i.e., if $n$ is even, then both sides are 0;
if odd, then $n = \sum_{d|n} \phi(d)$.
So
\[
  G_{\phi,\odd}(s) = \frac{G_\odd(s-1)}{G_\odd(s)}
  = \frac{\zeta(s-1)}{\zeta(s)} \frac{1 - 2^{-s + 1}}{1 - 2^{-s}}.
\]
...
\begin{align*}
  G_N(s - 1)
    = G_\phi(s) t(s)- G_{\phi, \odd}(s) \frac{t(s)-\zeta(s)}{2}
    =
...
\]
which is \refeq{cublyndon}, note that $\delta_{n,1} = \sum_{c|n}\mu(c)$.
%
\end{proof}
