...
The onset and bifurcation points of the $n$-cycles of
  ...
  connecting the cyclic polynomials of the periodic orbit points.
...
%
If $x_1$ deviates slightly from $x^*$
  and the sequence still converges to $x^*$,
  then it is \emph{stable}.
%
...
%
\big[the converse is, however, untrue, for a fixed point of $f^n$
  can also be a fixed point of $f^d$ for a divisor $d$ of $dn$:
  if $f^d(x) = x$, then $f^n(x) = f^d(\cdots f^d(x)\cdots) = x$\big],
%
%
and a cycle can be classified as stable or unstable
...
%
%
The orbit and cycle stability depend on the parameter $r$.
%
...
\[
  ...
  \end{array} \right),
\]
and the characteristic equation (\ref{eq:xr}) reads
\begin{equation}
...
\label{eq:xr1}
\end{equation}
Thus, the fixed point begins at $R_a = -1/4$ ($\lambda = +1$),
...
%
%
Setting the first factor (which is responsible for the 2-cycle,
  while the second factor is for unstable fixed points, see later)
  to zero yields $R = 1 - \lambda/4$;
...
we have six cyclic polynomials, and
%$C_0 = 1$,
%$C_1 = x_1 + \cdots + x_4$,
...
%
A simple elementary derivation of the 4-cycle is
...
The polynomial $A_n\Rlam$
  of the characteristic equation \refeq{xr}
  is derived for the fixed points of $f^n$,
  and thus encompasses cycles of shorter periods $d$ ($d|n$) as well.
%
...
For $n \ge 4$,
  $P_n\Rlam$ is readily recognized as
  the factor
  of $A_n\Rlam$
...
The number $\NB(n)$ of the square-free cyclic polynomials $C_p(\vx)$
  formed by the orbit points $\vx = \{x_1, \ldots, x_n\}$,
  which is also $\deg_\lambda A_n\Rlam$, is given by
\begin{equation}
  \deg_\lambda A_n\Rlam = \NB(n) = N(n) = (1/n) \textstyle\sum_{d|n} \varphi(n/d) 2^{\,d}.
\label{eq:necklace}
\end{equation}
%
\label{thm:necklace}
...
The degree in $\lambda$ of the minimal polynomial $P_n\Rlam$ of all $n$-cycles
  ...
  and is given by
\begin{equation}
  \deg_\lambda P_n\Rlam = L(n) = (1/n)\textstyle\sum_{d | n} \mu(n/d) 2^{\,d}.
\label{eq:lyndon}
\end{equation}
\label{thm:lyndon}
\end{theorem}
...
%
\begin{proof}
The number $L(n)$ of $n$-cycles is known \cite{hao, hao2, lutzky}.
Denote by $\lambda_c$ the value $\Lambda(\vx^c)$
  evaluated at the $c$th cycle points,
  then the minimal polynomial $P_n\Rlam$
  takes the form of $\prod_c (\lambda - \lambda_c)$.
Thus, $\deg_\lambda P_n\Rlam$ must be the same as $L(n)$.
\end{proof}
%
%
For the first few values, we have $L(n)$ =
...
A natural two-dimensional extension of the logistic map
...
$\vct J_b(x_1) \cdots \vct J_b(x_n)$ can be computed from
%
\begin{equation}
\big|\, \lambda \, \vct I - \vct J_b(x_1) \cdots \vct J_b(x_n) \,\big|
...
the product matrix $\vct J_b(x_1) \cdots \vct J_b(x_n)$,
respectively \cite{hitzl}.
In a stable cycle, the magnitude of $\lambda$
cannot exceed 1; and $\lambda = +1$ or $-1$
in \refeq{henonder} corresponds to
the onset or bifurcation point, respectively.
\refeq{henonder} is the counterpart of \refeq{der}.



Since the trace of the product of two matrices is
  independent of the order,
$\Theta(\vx)$ must a cyclic polynomial of $\vx = \{ x_k \}$.
%
...
\appendix{\label{apd:per4}Simple derivation of 4-cycles}
