...
At the onset and bifurcation points,
  ...
  linearly connected by \refeq{der}.
...
For convenience, we will drop the $R$ in ``$\Rlam$'' below till the end the next section.
...
where $\varphi(m)$ is Euler's totient function,
  ...
  $\varphi(6) = 2$ for 1 and 5 \cite{hardy}.
%
By \refeq{xr},
...
is the evaluation of the determinant $A_n\Rlam$ in the characteristic
equation (\ref{eq:xr}).
%
When implemented on a computer,
it is helpful to use Lagrange interpolation:
we evaluate $A_n\Rlam$ at $\deg_R A_n + 1$ values of $R$,
e.g., $R = 0, \pm1, \pm2,\ldots$, then piece them together
to a polynomial.
%
By using \refeq{primfac}, we only need $\deg_R P_n + 1$ values
