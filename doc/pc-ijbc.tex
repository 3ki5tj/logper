...
%
We attach the subscript $n$ to the determinant because
  it will later be used with those of divisors $d$ of $n$.
%
...
The first factor, denoted as $P_2\Rlam = 4R -r+\lambda$,
  ...
  while the second factor is for the fixed points (see \refsec{primfac}).
...
Since the discussions in this and the next sections
  ...
  we will drop the argument $R$ in $P_n\Rlam$, $A_n\Rlam$, etc.
%\begin{remark}
...
%\begin{theorem}
Next, the $n$-cycle polynomial $P_n\lam$ can be written as
\begin{equation}
...
%
One can show that i) such an intersection happens only
...
We can therefore distinguish \emph{original} cycles
  from those born out of the above intersection.
Similar to \refeq{QP},
...
Unlike the logistic map, this map is antisymmetric:
  $f(-x) = -f(x)$.
%
It can be mapped to another antisymmetric cubic map \cite{may2, brown3, brown4}
...
never reduces a cyclic polynomial with an odd (even) degree in $\vx$
...
%\end{theorem}
%
Since $N_e \approx N_3/2$, using the parity
  saves about half of the equations
  in computation.
%
...
The pure $n$-cycle polynomial $P_n\rlam$ can be obtained
  from \refeq{primfac} by proper substitutions
  $R\rightarrow r$.
...
%
The $\delta_{n,1}$ term corresponds to the fixed point
  at the origin,
%
%Excluding this term,
and
$L^*_e(n) \equiv L_e(n) - \delta_{n,1}$
 is the number of nontrivial $n$-cycles \cite{hao, hao2}.
For $n>1$, half of the cycles points $\vx$
...
The symmetry $f(-x) = -f(x)$ permits
  a particular \emph{half-cycle},
  that is a solution of $x_{n+1} = -x_1$,
  see \reffig{halfcycle} for examples.
%
A repeatd $n$-half-cycle is a $2n$-cycle:
...
%
We would have missed the $n$-half-cycle contributions
  in $A_{2n}\rlam$ and $P_{2n}\rlam$,
  had the characteristic equation been listed using odd,
  instead of even cyclic polynomials of $\vx$.
%The existence of half-cycles is why we use even, but not odd,
%  cyclic polynomials in the solution.


The polynomial $P^h_n\rlam$ for the $n$-half-cycles
  share the the same degree in $\lambda$ with $P_n\rlam$.
%
We further define $P^{h*}_n\lam$
  (omitting the argument $r$)
  as $P^h_n\lam$ for $n > 1$,
  but as $P^h_n\lam/(\lambda - r)$ for $n = 1$,
  to exclude the contribution from
...
%
For example, by solving the nontrivial 1-half-cycle or inversion point,
  $- x_1 = f(x_1)$, we have ${x_1}^2 = r + 1$,
...
As mentioned earily,
  ...
  derived for odd cyclic polynomials of $\vx$;
%
