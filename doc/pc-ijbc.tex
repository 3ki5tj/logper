...
The first factor is responsible for the 2-cycle,
  while the second factor is for the unstable fixed points. % (see later).
...
Thus, $B_d^c\Rlam$ is a polynomial of $\lambda$,
and $\deg_\lambda B_d^c\Rlam = \deg_\lambda A_d\Rlam$.
...
%
To show \refeq{primfac}, we first find the degrees in $\lambda$ of
  $A_n\Rlam$ [\refeq{necklace}] and $P_n\Rlam$ [\refeq{lyndon}].
...
the number $\NB(n)$ of the square-free cyclic polynomials $C_p$,
  %$(\vx)$ formed by the cycle points $\vx = \{x_1, \ldots, x_n\}$,
  is given by
\begin{equation}
...
By \refeq{xr},
  $\NB$ is also $\deg_\lambda A_n\Rlam$.
...
%\begin{theorem}
Next, the degree in $\lambda$ of the polynomial $P_n\Rlam$ of all $n$-cycles
...
%
%\begin{proof}
The number $\LB$ of $n$-cycles
  is well known \cite{hao, hao2, lutzky}.
The minimal polynomial $P_n\Rlam$
  can be written as $\prod_c (\lambda - \lambda_c)$,
where $\lambda_c$ is the value of $\Lambda(\vx^c)$
  evaluated at the $c$th cycle.
Thus, $\deg_\lambda P_n\Rlam = \LB$.
%\end{proof}
%
...
\end{equation}
%
Since the left side is also $\deg_\lambda A_n\Rlam$,
  and $A_n\Rlam$ has to represent all $d$-cycles with $d|n$,
$A_n\Rlam$
...
%  which is equal to
%  $\NB$ by \refeq{necklacelyndon}.
%Since $\deg_\lambda A_n\Rlam = \NB$ by \refeq{necklace},
...
%\begin{theorem}
We can now show that $P_n\Rlam$ and 
and $A_n\Rlam$ are related by
  \begin{equation}
    A_n\Rlam = \prod_{cd = n} Q_d^c\Rlam,
    \label{eq:prod}
  \end{equation}
  where
  $Q_d^c\Rlam = \prod_{k=1}^{c} P_d(R, e^{2 k \pi i/c} \lambda^{1/c})$
...
Since $Q_n^1\Rlam = P_n\Rlam$,
  ...
  in all $P_n\Rlam$ and $Q_d^c\Rlam$
  are also unities.
...
\refeq{primfac} is a corollary of \refeq{prod},
\begin{small}
\begin{align*}
 B_n^m (\lambda)
...
\begin{equation}
Q_n^m\Rlam
= \prod_{d|n} B_d^{m n/d}\Rlam^{\mu(n/d)},
\notag
%\tag{$\ref{eq:primfac}'$}
\end{equation}
...
\refeqsub{degR}{b} and (\ref{eq:degR}c) were known \cite{mira, stephenson1, blackhurst}.
%
\end{proof}
%
The first few $\deg_R A_n\Rlam$,
  $\deg_R P_n\Rlam$,
  and $\deg S_n(R)$ are
  1, 3, 6, 12, 20, 42, 70, 144, 270, 540, \dots,  %1034, 2112, 4108, \dots,
...
%
To filter out the contributions from shorter $d$-cycles with $d|n$,
  we repeat compute the characteristic equations for all divisors $d$
  and then apply \req{primfac}.
%


When implemented on a computer,
it is often helpful to evaluate $A_n(\lambda)$ by Lagrange interpolation,
...
Again, it encompasses the factors
  from the shorter $d$-cycles, as long as $d|n$.
The polynomial for the $n$-cycles can be obtained
  by \refeq{primfac} with proper substitutions, e.g.,
  $R\rightarrow r$.
