...
\newcommand{\reflem}[1]{Lemma \ref{thm:#1}}
...
\author{Cheng Zhang}
\address{Applied Physics Program and Department of Bioengineering,
...
\begin{abstract}
The onset and bifurcation points of $n$-cycles of
...
%
%
Thus, the stability of an $n$-cycle can be defined
  according to that of the corresponding fixed point of $f^n$:
  a stable $n$-cycle requires
...
%
and a stable cycle corresponds to a window of $r$
...
There are generally multiple such windows for a single $n$,
  ...
  and all $r_b$ zeros of another.
%
Our aim is thus to find the two polynomials for a given $n$.
%
The problem was tackled either by hand for small $n$
\cite{brown1, brown2, stephenson1, stephenson2, stephenson3,
      saha, bechhoefer, gordon, burm, zhang}
or by general computer-algebraic methods \cite{bailey1, bailey2, kk1, lewis}.
%
Below we present a specialized algorithm, which offers some performance gain.
%
%
%
...
The polynomials contain
  extraneous factors contributed by shorter $d$-cycles with $d|n$,
...
%
The square matrix $\mathbf A(R)$ formed by the coefficients $A_{pq}(R)$
...
A cyclic polynomial is an invariant
  under the cycling of the variables
...
For example, the above $a$ and $b$ are simple,
  and $x_1 x_2$ and $x_1 x_3$ are their generators, respectively.
%
...
Below we may drop %the subscript ``$_n$'' or even the argument
  ``$(\vx{n})$'' when $n$ is clear from the context.
...
The label $p$ of a simple square-free cyclic polynomial
  has no repeated index.
We denote the set of labels $p$ of all simple square-free cyclic polynomials by
...
%
%
The set of simple square-free cyclic polynomials
  furnishes a basis for expanding an arbitrary cyclic polynomial:
...
\]
  ...
  is the set of labels of all simple square-free cyclic polynomials,
...
The process will not last indefinitely
  ...
  by one.
Since both the replacement and the original polynomial are cyclic,
...
\reflem{sqrfree}
  ...
  as a combination of simple square-free ones.
The key step of the solution is to further show that
  at the onset and bifurcation points,
  the $\NB$ simple square-free cyclic polynomials $C_p$ are themselves
...
%
%
For any simple square-free cyclic polynomial $C_p(\vx{n})$,
the product $\Lambda(\vx{n}) \, C_p(\vx{n})$ is also cyclic,
  and can thus be expanded by \reflem{sqrfree} as
\begin{equation}
...
Since the set of homogeneous linear equations
...
we have only two simple square-free cyclic polynomials
$\{C_0, C_1\} = \{1, x_1\}$, with
$\Lambda = -2 \, x_1$. So
\[
  \Lambda
...
So $R = (\lambda^2 -2\lambda)/4$,
  and the fixed point becomes unstable at $R_b = 3/4$ where $\lambda = -1$.
...
$\Lambda \, C_1 %= 4 \, {x_1}^2 \, x_2 + 4 \, x_1 \, {x_2}^2
  = -8 \, R \, C_0 + 4 \, (R + 1) \, C_1$
by the example after \reflem{sqrfree},
$\Lambda \, C_{12}
...
So $R = 1 - \lambda/4$,
and the onset ($\lambda = +1$)
and bifurcation ($\lambda = -1$) values
are $R_a = 3/4$ and $R_b = 5/4$, respectively.
%
...
%
%
Only the first (sextic) factor, denoted as $P_4\Rlam$,
...
%  in the Appendix.
%
In the next section, we give a formula to systematically eliminate
...
Thus, it encompasses not just the $n$-cycles,
  but $d$-cycles for all divisors $d$ of $n$.
%
Fortunately, contributions from cycles of different periods
  are separate factors of $A_n\Rlam$.
For a large $n$,
  the factor $P_n\Rlam$ of the $n$-cycles only,
...
Since the above derivation works also for complex cycles,
  each complex root of $R$ of the polynomial $P_n\Rlam$ traces
...
%
%\begin{theorem}
the number $\NB(n)$ of square-free cyclic polynomials
...
\begin{equation}
  P_n\lam \equiv \textstyle\prod_l \Big[
    \Lambda_n\big(\vx{n}^{(l)}\big) - \lambda
  \Big],
\notag
\end{equation}
where $\vx{n}^{(l)}$ denotes the $l$th $n$-cycle,
and subscript $n$ is added to the function
$\Lambda$ for clarity.
%
Thus, $\deg_\lambda P_n\lam$
  is equal to the number $\LB(n)$ of $n$-cycles,
  which is given by
  \cite{lutzky, hao, hao2}
\begin{equation}
  %\deg_\lambda P_n\lam =
...
%\begin{theorem}
  \begin{equation}
    A_n\lam = \textstyle\prod_{cd = n} Q_d^c\lam,
    \label{eq:prod}
  \end{equation}
where $Q_d^c(\lambda) \equiv \prod_l
  \big[\Lambda_n\big(\vx{n}^{(l)}\big) - \lambda\big]$
  with $\vx{n}^{(l)}$ being the $n = cd$ consecutive points
  in the $l$th $d$-cycle,
%
  or the $d$-cycle points $\vx{d}^{(l)}$
  repeated for $c$ times:
\begin{equation}
  \vx{n}^{(l)}
  =
  \vx{d}^{(l)} \times c
...
%
It follows that
$\Lambda_n(\vx{n}) = \left[ \Lambda_d(\vx{d}) \right]^c$.
%
By factoring ${\Lambda_d}^c - \lambda$
...
%
This factorization is due to the intersection between
  the $n$-cycle and a shorter $d$-cycle with $d|n$.
%
The intersection forces the two cycles to share the orbit
...
As a result, the $n$-cycle polynomial $P_n(R, \lambda)$
  has to include the $d$-cycle polynomial $P_d(R, \lambda')$ as a factor,
...
We can thus distinguish \emph{original} cycles
  from those born out of the above period-tupling.
%
Similar to \refeq{QP},
  the unoriginal cycles
  from the shorter $d$-cycles with $d|n$
  contribute a cyclotomic product
  $\prod_{(k, c) = 1}
      P_{d}
        \left(
          R, e^{2k\pi i/c}
...
With these contributions removed,
  we obtain the polynomial responsible for
  the onset of the original $n$-cycles:
  \begin{equation}
    S_n(R)
    = \frac
    {
...
%
In implementation,
we found it helpful to compute the polynomial by interpolation.
That is, we first evaluate $A_n\Rlam$ at $\deg_R A_n + 1$ values of $R$,
e.g., $R = 0, \pm1, \pm2,\ldots$, then piece them together
into a polynomial of $R$.
%
Further, we can directly evaluate $P_n\Rlam$ by \refeq{primfac},
  and thus reduce the number of evaluations to $\deg_R P_n + 1$.
%
%
Here,
...
In implementing the resultant method on Magma, we have coded a version
  that used the polynomial-interpolation technique
...
Since the resultant contains
  only the $n$th power of the $n$-cycle polynomial $P_n\Rlam$,
the number of evaluation points needs to be doubled
  to recover the sign of $P_n\Rlam$ for an even $n$.
%
The current method works better than the resultant method
...
\begin{figure}[h]
  ...
  Time to compute the $n$-cycle bifurcation polynomials,
    ...
    an Intel\textsuperscript{\textregistered} 2.5GHz Dual-Core CPU
...
we discuss two extensions of the above method.
...
%
  ...
  in terms of the simple square-free cyclic polynomials,
...
The enlarged basis significantly increases the dimension of
  the coefficient matrix in \refeq{xr}, hence the computational cost.
%
...
%
First, we may include in the basis
  about only half of the simple cyclic polynomials,
  that is, those of \emph{even} degrees in $\vx{n}$.
%
This is because \refeq{cubreplace} %contains only linear and cubic terms,
  never reduces even-degree polynomials to odd-degree ones,
  or the other way around.
%
The characteristic equation obtained
  from the basis of even-degree cyclic polynomials
  is the same as that
  from the basis of odd-degree cyclic polynomials for an odd $n$.
But for an even $n$, the former admits several symmetric cycles
  that are absent in the latter.
The difference is due to the symmetry of \refeq{cubic},
...
%
One can verify that $\tilde P^*_n\rlam$ is the same as the $n$-cycle polynomial
  derived from the basis of odd-degree cyclic polynomials.
%



We now calculate the degrees of the above polynomials
  to facilitate the polynomial-interpolation technique
...
%
which is also the number of $n$-cycles \cite{zeng},
%
and
...
To find the degrees of $\tilde P^*_n\rlam$ and $H^*_n\rlam$,
  we consider the quadratic transformation $u_k = {x_k}^2/r$,
...
%
So $\tilde P^*_n(r, \lambda) = \tilde P^*_n(-r, \lambda)$,
...
Thus, the above polynomials need to be evaluated only
  ...
  in the polynomial-interpolation technique.
%
