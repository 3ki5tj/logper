...
\big[the converse is, however, untrue, for a fixed point of $f^n$
  can also be a fixed point of $f^d$ of a divisor $d$ of $dn$:
...
Note that a cyclic polynomial is not necessarily a symmetric one,
...
We now consider
  a subset of \emph{square-free} cyclic polynomials,
  which have no square or higher powers of $x_k$.
For example, $C_{12}(\vx) = x_1 x_2 + \cdots$ is square-free,
...
and its size $|\Bset|$ equals the number $\NB$ of
  square-free cyclic polynomials.
%
%
The square-free cyclic polynomials
  furnish a basis for expanding cyclic polynomials:
...
Since the original polynomial is cyclic,
  so is the reduced one, which is also square-free.
...
The matrix of the connecting coefficients must have a zero determinant,
...
\label{eq:xr}
\end{equation}
Here, $A_n\RX$ is a polynomial of $R$ and $\lambda$,
  and the subscript $n$ is attached such that
  it can be used later with $A_d\RX$ of divisors $d$ of $n$.
...
\begin{theorem}
  ...
  and $T_{pq}(R)$ are the coefficients
...
%\begin{remark}[1]
  ...
  for stable cycles.
  These regions are bulbs in the inverted Mandelbrot set,
...
the method can also compute the region of stability for $R$
  (i.e., the inverted Mandelbrot set),
...
On the complex domain, however, we can have a $c$-fold branching with $c>2$,
  ...
  in the inverted Mandelbrot set, see \reffig{logbulb}.
\end{remark}
